...
trigger (mean finger curl from bone angles) and grip (thumb closure), with EMA smoothing, a 0.5 s
hold on tracking loss and controller fallback per side, and — via `InspireBridge(get_targets=...)` —
per-finger closures `[little, ring, middle, index, thumb_bend, thumb_rot]` so each finger moves
independently (force rule applied per finger). Buttons and sticks stay on the controllers.
